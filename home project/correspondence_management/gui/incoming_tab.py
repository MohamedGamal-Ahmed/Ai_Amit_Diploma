...
class IncomingTab:
    ...
    def create_widgets(self):
        ...
        if self.auth_manager.has_permission('add_incoming'):
            add_btn = ttk.Button(
                buttons_frame,
                text="إضافة مراسلة",
        ...
        if self.auth_manager.has_permission('edit_incoming'):
            edit_btn = ttk.Button(
                buttons_frame,
                text="تعديل",
        ...
        if self.auth_manager.has_permission('delete_incoming'):
    ...
    def create_table(self, parent):
        ...
        # تعريف الأعمدة
        columns = (
            'id', 'reference_number', 'subject_code', 'subject', 'sender', 
            'sender_department', 'responsible_person', 'received_date', 'priority', 'status'
        )
        ...
        self.tree.heading('sender', text='المرسل')
        self.tree.heading('sender_department', text='الجهة')
        self.tree.heading('responsible_person', text='المسئول')
        ...
        self.tree.column('subject', width=250, anchor='e')
        self.tree.column('sender', width=120, anchor='e')
        self.tree.column('sender_department', width=120, anchor='e')
        self.tree.column('responsible_person', width=120, anchor='e')
        self.tree.column('received_date', width=100, anchor='center')
    ...
    def refresh_data(self):
        ...
        query = '''
            SELECT id, reference_number, subject_code, subject, sender, sender_department,
                   responsible_person, received_date, priority, status, content, notes
        ...
        if search_term:
            query = '''
                SELECT id, reference_number, subject_code, subject, sender, sender_department,
                       responsible_person, received_date, priority, status, content, notes
                FROM incoming_correspondence
                WHERE reference_number LIKE ? OR subject LIKE ? OR sender LIKE ? OR subject_code LIKE ?
                ORDER BY received_date DESC, id DESC
            '''
    ...
    def show_context_menu(self, event):
        ...
        if self.auth_manager.has_permission('add_outgoing'):
            context_menu.add_command(label="رد على المراسلة", command=self.reply_correspondence)
    ...
    def add_correspondence(self):
        ...
        try:
            ...
            if hasattr(self.parent, 'show_notification'):
                self.parent.show_notification(f"فشل في فتح نموذج إضافة المراسلة: {e}", type_="error")
            else:
                messagebox.showerror("خطأ", f"فشل في فتح نموذج إضافة المراسلة: {e}")
    
    def edit_correspondence(self):
        """تعديل المراسلة الم��ددة"""
        selected = self.tree.selection()
        if not selected:
            if hasattr(self.parent, 'show_notification'):
                self.parent.show_notification("يرجى اختيار مراسلة للتعديل", type_="warning")
            else:
        ...
        try:
            ...
            if hasattr(self.parent, 'show_notification'):
                self.parent.show_notification(f"فشل في فتح نموذج تعديل المراسلة: {e}", type_="error")
            else:
    ...
    def delete_correspondence(self):
        ...
        # حذف المراسلة
        query = "DELETE FROM incoming_correspondence WHERE id = ?"
        ...
        if result is not None:
            ...
            self.db_manager.log_activity(
                ...
                action=f"حذف مراسلة واردة رقم {correspondence_id}",
    ...
    def add_followup(self):
        ...
        if not selected:
            if hasattr(self.parent, 'show_notification'):
                self.parent.show_notification("يرجى اختيار مراسلة لإضافة متابعة", type_="warning")
            else:
    ...
    def reply_correspondence(self):
        """الرد على المراسلة"""
        selected = self.tree.selection()
        if not selected:
            if hasattr(self.parent, 'show_notification'):
                self.parent.show_notification("يرجى اختيار مراسلة للرد عليها", type_="warning")
            else:
                messagebox.showwarning("تحذير", "يرجى اختيار مراسلة للرد عليها")
            return
        
        item = self.tree.item(selected[0])
        correspondence_id = item['values'][0]
        
        # جلب بيانات المراسلة الأصلية
        query = "SELECT * FROM incoming_correspondence WHERE id = ?"
        data = self.db_manager.execute_query(query, (correspondence_id,))
        
        if data:
            original = dict(data[0])
            
            from gui.correspondence_form import CorrespondenceForm
            CorrespondenceForm(
                self.frame,
                self.db_manager,
                self.auth_manager,
                ...
                related_incoming_id=correspondence_id,
